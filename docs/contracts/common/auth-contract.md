...
## 1. Token type
- **Algorithm:** RS256 (bất đối xứng). auth-service giữ **private key** (ký); service khác **verify-only** bằng **public key**.
...
- **Public key distribution:** service verify cần public key. Dev: keypair throwaway commit (classpath). Prod: nạp qua `JWT_PUBLIC_KEY_PATH`; **private key KHÔNG commit** (`.gitignore *-prod*.pem`).

## 1b. Token transport (body + cookie)
- **access token:** trả trong **body** login → client lưu **memory** → gắn `Authorization: Bearer` trên **mọi call**. Đây là đường auth **cross-service THẬT** (service verify HEADER, không cookie).
...
```
Verify chữ ký + `exp` TRƯỚC khi tin claims; sai → 401 `INVALID_TOKEN`.
...
Pattern TƯƠNG LAI (nếu gateway terminate auth + inject header → downstream trust header):
```http
X-User-ID: <uuid>
X-User-Roles: AUDIENCE,ORGANIZER
X-Request-ID: <request-id>
```
- `X-Request-ID` (tracing) thì **đã dùng** (echo + propagate). `X-User-*` là forward-looking, chưa áp dụng.
...
- **Service nào kiểm tra blacklist:** ⚠️ **HIỆN TẠI chỉ auth-service** (`JwtAuthenticationFilter`) check blacklist trên endpoint của nó. Service khác (inventory/order) **verify-only** (chữ ký + `exp`) — **KHÔNG check blacklist** → một access token đã logout vẫn được inventory/order chấp nhận **tới khi hết hạn (≤15')**. Gateway-side blacklist check = 🔭 PLANNED (ADR-AUTH-003, parked).
