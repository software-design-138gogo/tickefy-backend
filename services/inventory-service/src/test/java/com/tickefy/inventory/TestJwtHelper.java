...
/**
 * Signs RS256 JWT tokens for integration tests.
 * Uses the SAME private key as auth-service (same keypair as inventory jwt-dev-public.pem).
 * Private key embedded here — test-only, never shipped in production artifact.
 */
public class TestJwtHelper {

    // This is jwt-dev-private.pem from auth-service/src/main/resources/keys/
    // Same keypair as inventory-service/src/main/resources/keys/jwt-dev-public.pem
    ...
    /**
     * Generate a valid RS256 JWT token for testing.
     *
     * @param userId   subject (user UUID as string)
     * @param roles    list of role names (without ROLE_ prefix, e.g. "ORGANIZER", "AUDIENCE")
     * @param ttlMs    token TTL in milliseconds from now
     */
    ...
    /** Shortcut: token valid 15 minutes. */
    ...
    /** Tampered token: just append garbage so signature is invalid. */
